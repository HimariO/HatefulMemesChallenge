...
class Compose(object):
    def __init__(self, transforms):
        ...
        self.ordered_keys = ['image', 'bboxes', 'masks', 'im_info', 'flipped']
    
    def HWC2CHW(self, image):
        return torch.Tensor(image).permute(2, 0, 1)
    
    def CHW2HWC(self, image):
        return image.permute(1, 2, 0).numpy()

    def __call__(self, image, boxes, masks, im_info, flipped):
        for T in self.transforms:
            if isinstance(T, (album.BasicTransform, album.Compose)):
                image = self.CHW2HWC(image)
                datas = [image, boxes.numpy().tolist(), masks, im_info, flipped]
                data_dict = {k: v for k, v in zip(self.ordered_keys, datas)}
                data_dict['fake_label'] = [0] * len(boxes)
                
                data_dict = T(**data_dict)
                
                image, boxes, masks, im_info, flipped = (data_dict[k] for k in self.ordered_keys)
