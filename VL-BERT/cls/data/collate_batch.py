...
class BatchCollator(object):
    ...
    def __call__(self, batch):
        ...
        if 'text_tags' in self.data_names:
            self.use_v2_data = True
            max_img_boxes = max([
                data[self.data_names.index('img_boxes')].shape[0]
                for data in batch
            ])
        else:
            max_img_boxes = None
        ...
        for i, ibatch in enumerate(batch):
            ...
            if self.use_v2_data:
                text_tags = ibatch[self.data_names.index('text_tags')]
                out['text_tags'] = clip_pad_1d(text_tags, max_text_length, pad=0)
                
                img_boxes = ibatch[self.data_names.index('img_boxes')]
                out['img_boxes'] = clip_pad_boxes(img_boxes, max_img_boxes, pad=-2)
