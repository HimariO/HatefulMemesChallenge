...
class ValidationMonitor(object):
    def __init__(self, val_func, val_loader, metrics, host_metric_name='Acc', label_index_in_batch=-1, model_dir=None):
    ...
    def __call__(self, epoch_num, net, optimizer, writer):
        self.val_func(
            net, self.val_loader, self.metrics, self.label_index_in_batch,
            model_dir=self.model_dir, epoch_num=epoch_num)
