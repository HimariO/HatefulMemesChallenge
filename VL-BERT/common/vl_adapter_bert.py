...
from external.adapter_transformers.modeling_bert import BertLayerNorm, BertEncoder, BertPooler, ACT2FN, BertOnlyMLMHead
from external.adapter_transformers import AdapterType, ModelAdaptersConfig, PfeifferConfig
...
class VisualLinguisticBert(BaseModel):
    ...
    def setup_adapter(self):
        ...
        adapter_name = 'vl_meme'
        self.encoder.config.adapters = ModelAdaptersConfig()
        self.encoder.config.adapters.add(adapter_name, AdapterType.text_task, config=PfeifferConfig())

        self.encoder.add_adapter(adapter_name, AdapterType.text_task)
        self.encoder.enable_adapters([adapter_name], False, True)
    ...
    def forward(self,
        ...
        if output_attention_probs:
            _, encoded_layers, attention_probs = self.encoder(embedding_output,
                                                           extended_attention_mask,
                                                           output_all_encoded_layers=True,
                                                           output_attention_probs=output_attention_probs)
        else:
            _, encoded_layers = self.encoder(embedding_output,
                                          extended_attention_mask,
