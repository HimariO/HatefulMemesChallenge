...
class DetectFeatTxtTokDataset(Dataset):
    def __init__(self, txt_db, img_db):
        ...
        for tl, id_ in zip(txt_lens, self.ids):
            try:
                self.img_db.name2nbb[txt2img[id_]]
...
class ConcatDatasetWithLens(ConcatDataset):
    ...
    def weights_by_class(self):
        dataset_weight = [sum(self.lens) / l for l in self.lens]
        weights = [
            [_d * dw for _d in d.weights_by_class]
            for d, dw in zip(self.datasets, dataset_weight)
        ]
