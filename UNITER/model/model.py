...
class UniterModel(UniterPreTrainedModel):
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
        ...
        embedding_output = torch.gather(
            torch.cat([txt_emb, img_emb], dim=1),
...
class VillaModel(UniterModel):

    def forward(self, input_ids, position_ids,
                img_feat, img_pos_feat,
                attention_mask, gather_index=None, img_masks=None,
                output_all_encoded_layers=True,
                txt_type_ids=None, img_type_ids=None, embed_only=False):
        if embed_only:
            return self.forward_embed(
                input_ids, position_ids,
                img_feat, img_pos_feat, attention_mask,
                img_masks=img_masks,
                txt_type_ids=txt_type_ids, img_type_ids=img_type_ids)
        else:
    ...
    def forward_embed(self, input_ids, position_ids,
                    img_feat, img_pos_feat,
                    attention_mask, img_masks=None,
                    txt_type_ids=None, img_type_ids=None):
        # compute self-attention mask
        extended_attention_mask = attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(
        ...
        if input_ids is None:
            ...
            embedding_img = self._compute_img_embeddings(
                img_feat, img_pos_feat, img_masks, img_type_ids)
            
            embedding_txt = self._compute_txt_embeddings(
                input_ids, position_ids, txt_type_ids)
            embedding_output = {
                'img': embedding_img,
                'txt': embedding_txt,
    ...
    def forward_encode(self, embedding_output, extended_attention_mask,
                        gather_index=None, output_all_encoded_layers=True):
        if isinstance(embedding_output, dict):
            txt_emb = embedding_output['txt']
            img_emb = embedding_output['img']
